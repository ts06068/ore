...
async def main():
 ...
 async with httpx.AsyncClient(base_url='http://127.0.0.1:8765',headers={'Authorization':'Bearer '+Path('.ore/operator.token').read_text().strip()},timeout=120) as c:
  ...
  try:await post(base+'/input',{'action':'click','selector':'button:has-text("Reject all")','epoch':epoch})
  except httpx.HTTPStatusError:pass
  for selector,value in [('input[name="appid"]','ore-retrieval-20260910'),('input[name="appname"]','ORE'),('textarea[name="appdesc"]','Self-hosted research application for scholarly metadata search, DOI discovery, citation metadata and authorized retrieval. API credentials are stored server-side. Personal research use; no resale. Local service: http://localhost:8765.')]:
  ...
  await post(base+'/secret-capture',{'selector':'body','ref':'onboarding/clarivate-application-page','epoch':epoch})
  secret=SecretStore(Settings().prepare().state_dir);body=secret.get('onboarding/clarivate-application-page')
  safe=re.sub(r'[A-Za-z0-9._~+/=\-]{24,}','[private identifier]',body)
  email=secret.get('onboarding/email')
  if email:safe=safe.replace(email,'[private account]')
  result={'application_id':'ore-retrieval-20260910','text':safe[:13000]};Path('.ore/reports/clarivate-application-result.json').write_text(json.dumps(result,indent=2));print(json.dumps(result))
asyncio.run(main())
