...
export class OreClient {
  ...
  connectionSecret(card:ConnectionCard,field:string,value:string,idempotencyKey=crypto.randomUUID(),options:{credential_id?:string}={}):Promise<ConnectionCard> {return this.request(`/v1/connections/${segment(card.id)}/secret`,{method:'POST',body:JSON.stringify({field,value,expected_version:card.state_version,idempotency_key:idempotencyKey,...options})});}
