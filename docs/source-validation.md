...
**WoS Starter is now submitted and awaiting provider approval.** After the initial Gmail route encountered an institutional-email condition, the user supplied credentials for an existing Yonsei institutional account. Its login and institutional email were confirmed privately, the developer profile was registered, and application ORE (`ore-yonsei-20260910`) was created. Its **Free Institutional Member Plan** request was submitted successfully at 12:03:58 UTC. The actual portal states `Subscription approval is pending`; it displays a quota of 5 requests/second and 5,000/day after approval. No key or WoS API query has been verified. Evidence: `.ore/reports/wos-onboarding-status.json` and `.ore/reports/clarivate-institution-subscription-result.json`.

The supplied affiliation is Yonsei University. The old Gmail application remains unsubscribed, with no account merge/deletion. The earlier email-addition request is superseded; no further email-addition verification is required for this application.

A separate operator-driven ORE browser check displayed **Yonsei University**, then an actual Core Collection query `DO=(10.4258/hir.2024.30.3.266)` returned one matching article, accession `WOS:001292832000010`, with a publisher full-text link visible. This establishes bounded browser search access; no export file or publisher PDF was fetched in that probe, and no API entitlement is inferred. Evidence: `.ore/reports/wos-browser-live-validation.json`.
