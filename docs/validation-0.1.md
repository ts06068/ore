> Historical 0.1 execution record. Current results are in [VALIDATION](../VALIDATION.md). Relative source paths below refer to the repository root.
...
| Scopus API | User-authorized Elsevier key issued; one actual native exact-DOI STANDARD search returned one matching record / total 1. No returned content sent to an LLM in this probe. | Local operator probe; `.ore/reports/scopus-live-validation.json` |
