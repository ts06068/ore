    ...
    def _builtins(self):
        ...
        for name, model in ACTION_MODELS.items():
            ...
            self.register(ToolSpec(name, TOOLS[name], model.model_json_schema(), read_only=name in readonly,
                ...
                handler_digest=canonical_digest({file:hashlib.sha256((Path(__file__).parent/file).read_bytes()).hexdigest() for file in ('tools.py','contracts.py','vault.py','browser.py', *(['browser_resume.py','native_scope.py'] if name in {'state','browser_open','browser_observe','browser_action','download','artifact_commit','page_extract','challenge','handoff'} else []), *(['provider_enrollment.py','connection_agent.py'] if name == 'provider_form' else []))})), legacy)
