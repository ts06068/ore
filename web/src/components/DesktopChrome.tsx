...
import {errorMessage,record} from '../lib/format';
...
export default function DesktopChrome({client,handoff,onAttached}:Props){
 ...
 const historical=isHistoricalHandoff(handoff);
 const planning=record(handoff.conversation_context).phase==='planning';
 ...
 return <section className="panel" aria-label="ORE Chrome desktop">
  ...
  {attached?<><p role="status">ORE Chrome desktop is attached. Complete any access checks in the browser below, then use {planning?'Verify planning access':'Verify & resume'}.</p><button className="button outline" disabled={busy} onClick={()=>void change('close')}>{busy&&<Spinner/>}Close virtual Chrome</button></>:<button className="button primary" disabled={busy||!safeExternalCheckpoint(handoff.checkpoint_url)||preview?.eligible!==true||preview.expected_version!==handoff.state_version} onClick={()=>void change('attach')}>{busy?<Spinner/>:<Monitor size={16}/>}Use ORE Chrome desktop</button>}
