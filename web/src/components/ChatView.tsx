...
import type {Artifact, ConnectionCard, Conversation, ConversationEvent, JsonObject, Model, OreClient, OreEvent, PlanRevision, SchedulerSnapshot, WorkflowRun} from '@ore/sdk';
...
export default function ChatView({client,id,models,onNavigate,onChanged}:Props){
 ...
 const apiKeyRef=selectedApiConnection?(record(selectedApiConnection.backend).api_key_ref??selectedApiConnection.api_key_ref):record(record(conversation?.settings).backend).api_key_ref;
 ...
 useEffect(()=>{const settings=record(conversation?.settings);if(settings.mode==='plan'||settings.mode==='execute')setMode(settings.mode);if(settings.model_policy==='auto'||settings.model_policy==='fixed')setPolicy(settings.model_policy);if(typeof settings.model==='string')setModel(settings.model);if(typeof settings.reasoning_effort==='string')setEffort(settings.reasoning_effort);const backend=record(settings.backend);setBackendKind(text(backend.kind,'codex'));if(backend.kind&&backend.kind!=='codex'){const selected=text(backend.model??settings.model,backend.kind==='claude_code'?'sonnet':'');setProviderModel(['sonnet','opus'].includes(selected)?selected:'custom');setProviderCustom(selected);}},[conversation?.id]);
 ...
 async function send(){const content=draft.trim();if(!content||sending||invalidProvider)return;setSending(true);setError('');try{const settings=backendKind!=='codex'?{mode,model_policy:'fixed' as const,model:providerModelId,backend:{kind:backendKind,model:providerModelId,...(apiBackend?{api_key_ref:apiKeyRef}:{})}}:{mode,model_policy:policy,...(record(record(conversation?.settings).backend).kind&&record(record(conversation?.settings).backend).kind!=='codex'?{backend:{kind:'codex'}}:{}),...(policy==='fixed'?{model,reasoning_effort:effort||undefined}:{})};const target=id||((await client.createConversation(settings)).id);if(!id)onNavigate('/chat/'+encodeURIComponent(target));const fresh=await client.sendMessage(target,{content,...settings,...(continueChanges&&mode==='execute'&&conversation?.approved_plan_id&&!paused&&!stopping?{change_and_continue:true}:{})});if(!id||idRef.current===target){acceptSnapshot(fresh);setDraft('');setContinueChanges(false);followTail.current=true;}changedRef.current();}catch(cause){setError(errorMessage(cause));}finally{setSending(false);}}
 ...
 return <section className={`chat-workspace ${resultsOpen?'chat-with-results':''}`} aria-label="ORE conversation">
  ...
  <div className="chat-transcript" ref={transcriptRef} onScroll={()=>{const element=transcriptRef.current;if(element){followTail.current=element.scrollHeight-element.scrollTop-element.clientHeight<100;if(followTail.current)setShowLatest(false);}}}>
   <div className="chat-column">
    ...
    <ConnectionCards client={client} conversationId={id||undefined} open={connectionsOpen} onClose={()=>setConnectionsOpen(false)} onNavigate={onNavigate} refreshKey={conversation?.events_cursor} onConnectionsChange={setConnectionCards} agentBackend={backendKind} agentModel={backendKind==='codex'?model:providerModelId}/>
